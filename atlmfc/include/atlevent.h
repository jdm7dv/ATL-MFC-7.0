...
namespace ATL {
    ...
    public:
        ...
        HRESULT __WhichThis(U* /*pU*/, IUnknown* pS, T* pThis, int nSize, bool bNext, int idx) {
            if (bNext) {
                 if (__pCurrent != 0) {
                     __pCurrent = __pCurrent->__pNext;
                 }
        ...
        template <typename U /*interface*/>
        HRESULT __AddHandler(U* pU, T* pThis, int idxSink, IUnknown* pS, int idx, int nSize) {
            if (pS == 0 || pThis == 0) {
                return E_FAIL;
            }
            bool bNext = false;
            bool bDone = false;
            while (!bDone) {
                HRESULT hr = __WhichThis(pU, pS, pThis, nSize, bNext, idx);
                if (SUCCEEDED(hr)) {
        ...
        template <typename U /*interface*/>
        HRESULT __RemoveHandler(U* pU, T* pThis, IUnknown* pS, int idx) {
            bool bNext = false;
            bool bDone = false;
            while (!bDone) {
                __WhichThis(pU, pS, pThis, 0, bNext, idx);
                if (__pCurrent == 0) {
                    return E_FAIL;
                }
                if (__pCurrent->__proxyIndex[idx] != __InvalidIndex) {
