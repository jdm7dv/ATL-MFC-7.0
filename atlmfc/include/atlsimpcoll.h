...
template <class T, class TEqual = CSimpleArrayEqualHelper< T > >
class CSimpleValArray : public CSimpleArray< T, TEqual >
{
public:
	typedef T _ArrayElementType;

// Construction/destruction
	CSimpleValArray() : CSimpleArray< T, TEqual >()
	{ }

	CSimpleValArray(const CSimpleValArray< T, TEqual >& src) : CSimpleArray< T, TEqual >(src)
	{
	}
	CSimpleValArray< T, TEqual >& operator=(const CSimpleValArray< T, TEqual >& src)
	{
		CSimpleArray< T, TEqual >::operator=(src);
		return *this;
	}
...
// Operations
	BOOL Add(const T &t)
	{
		return CSimpleArray< T, TEqual >::Add(t);
	}
	BOOL Remove(const T &t)
	{
		return CSimpleArray< T, TEqual >::Remove(t);
	}
	
	// REVIEW: why does this exist? (i.e., handled by CSimpleArray)
	/*const T& operator[] (int nIndex) const
	{
		return CSimpleArray< T, TEqual >::operator[](nIndex);
	}*/
};
...
// Implementation
	...
	void SetAtIndex(int nIndex, const TKey& key, const TVal& val)
	{
		ATLASSERT(nIndex >= 0 && nIndex < m_nSize);
