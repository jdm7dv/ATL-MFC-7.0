...
inline ULONG _QIThunk::Release()
{
	...
	if (m_bBreak)
		DebugBreak();
	ATLASSERT(m_dwRef > 0);
	ULONG l = AtlInterlockedDecrement(&m_dwRef);
	ATLTRACE2(atlTraceRefcount, 0, _T("QIThunk - %-10d\tRelease :\tObject = 0x%08x\tRefcount = %d\t"), m_nIndex, m_pUnk, m_dwRef);
	AtlDumpIID(m_iid, m_lpszClassName, S_OK);
	bool bDeleteThunk = (l == 0 && !m_bNonAddRefThunk);
	m_pUnk->Release();
	if (bDeleteThunk)
...
{
	return _AtlWinModule.m_csWindowCreate;
}

inline void CComModule::EnterWindowCriticalSection()
{
	_AtlWinModule.EnterWindowCriticalSection();
}

inline void CComModule::LeaveWindowCriticalSection()
{
...
{
	return _AtlComModule.m_csObjMap;
}
	...
	if (m_pObjMap != NULL)
	{
		...
		while (pEntry->pclsid != NULL)
		{
			if ((pEntry->pfnGetClassObject != NULL) && InlineIsEqualGUID(rclsid, *pEntry->pclsid))
			{
				if (pEntry->pCF == NULL)
				{
					CCritSecLock lock(_AtlComModule.m_csObjMap);
					if (pEntry->pCF == NULL)
