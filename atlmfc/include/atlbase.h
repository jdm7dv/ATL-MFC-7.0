...
// REVIEW: remove when new crtdbg.h is in place
#ifndef _ASSERT_BASE
...
public:
	...
	bool m_bNT5orWin98;
	bool m_bNT;
	CSimpleArray<ATOM>* m_paWindowClassAtoms;	
};
...
#pragma pack(push,8)
extern "C" LRESULT CALLBACK _StdCallThunkProc( HWND, UINT, WPARAM, LPARAM );
struct _FuncDesc
{
	void* pfn;
	void* gp;
...
struct _stdcallthunk
{
	...
	void Init(DWORD_PTR proc, void* pThis)
	{
		...
		pThunkProc = reinterpret_cast< const _FuncDesc* >( _StdCallThunkProc );
...
// Win95's InterlockedIncrement/InterlockedDecrement only return -1, 0, or 1,
// because the 386 can't do any better than that.  Win98 and NT4+ return the
// actual new value.  We depend on this behavior in the debug refcount code,
// so if we're building _DEBUG, we implement these interlocked operations ourself.
// Note that this means that your _DEBUG code won't run on a 386.
inline LONG AtlInterlockedIncrement( LONG* pn ) throw()
{
#if defined(_DEBUG) && defined(_M_IX86) && !defined(_ATL_USE_REAL_INTERLOCKED)
	LONG nRet;
	...
	__asm
	{
		...
		// If you get an invalid opcode fault on the following line, it means
		// that you're running on a 386 CPU.  To enable your application run on a
		// 386, you can #define _ATL_USE_REAL_INTERLOCKED before including any
		// ATL headers.  This will, however, cause ATL's refcount debugging support 
		// to work incorrectly on Windows 95.
		lock xadd [ecx], eax; 
		inc eax;
		mov nRet, eax;
	}
	...
	return( nRet );
#else
	return( ::InterlockedIncrement( pn ) );
#endif
}

inline LONG AtlInterlockedDecrement( LONG* pn ) throw()
{
#if defined(_DEBUG) && defined(_M_IX86) && !defined(_ATL_USE_REAL_INTERLOCKED)
	LONG nRet;
	...
	return( nRet );
#else
	return( ::InterlockedDecrement( pn ) );
#endif
}
...
private:
	ATL_NOINLINE void AllocateHeap( size_t nBytes )
	{
		...
		p = static_cast< T* >( Allocator::Allocate( nBytes ) );
		if( p == NULL )
		{
...
{
public:
	static ULONG WINAPI Increment(LPLONG p) throw() {return AtlInterlockedIncrement(p);}
	static ULONG WINAPI Decrement(LPLONG p) throw() {return AtlInterlockedDecrement(p);}
...
struct _QIThunk
{
	...
	ULONG InternalAddRef()
	{
		...
		if (m_bBreak)
			DebugBreak();
		ATLASSERT(m_dwRef >= 0);
		long l = AtlInterlockedIncrement(&m_dwRef);
		ATLTRACE(atlTraceRefcount, 2, _T("QIThunk - %-10d\tAddRef  :\tObject = 0x%08x\tRefcount = %d\t"), m_nIndex, m_pUnk, m_dwRef);
...
extern "C" _IMAGE_FILE_HEADER __ImageBase;
...
public :
	...
	{
		//REVIEW
		return static_cast< HINSTANCE >(InterlockedExchangePointer(reinterpret_cast< void** >(&m_hInstResource), reinterpret_cast< void* >(reinterpret_cast< LONG_PTR >( hInst ))));
	}

	bool AddResourceInstance(HINSTANCE hInst) throw()
	{
		CCritSecLock lock(m_csResource);
...
inline const ATLSTRINGRESOURCEIMAGE* AtlGetStringResourceImage( UINT id )
{
	...
	for (int i = 1; hInst != NULL && p == NULL; i++)
	{
...
inline HINSTANCE AtlFindResourceInstance(LPCTSTR lpName, LPCTSTR lpType) throw()
{
...
inline HINSTANCE AtlFindResourceInstance(UINT nID, LPCTSTR lpType) throw()
{
	return AtlFindResourceInstance(MAKEINTRESOURCE(nID), lpType);
}

inline HINSTANCE AtlFindResourceInstanceEx(LPCTSTR lpName, LPCTSTR lpType, WORD wLanguage) throw()
{
	ATLASSERT(lpType != RT_STRING);	// Call AtlGetStringResourceImage to get the string
	if (lpType == RT_STRING)
		return NULL;
	...
	for (int i = 1; hInst != NULL; hInst = _AtlBaseModule.GetHInstanceAt(i++))
	{
		hResource = ::FindResourceEx(hInst, lpName, lpType, wLanguage);
		if (hResource != NULL)
		{
			return hInst;
		}
...
inline HINSTANCE AtlFindResourceInstanceEx(UINT nID, LPCTSTR lpType, WORD wLanguage) throw()
{
	return AtlFindResourceInstanceEx(MAKEINTRESOURCE(nID), lpType, wLanguage);
}
...
public:
	...
	HRESULT AddThunk(IUnknown** pp, LPCTSTR lpsz, REFIID iid)
	{
		...
		if (InlineIsEqualUnknown(iid))
		{
			...
			{
				if (m_aThunks.operator[](i)->m_pUnk == p && InlineIsEqualGUID(m_aThunks.operator[](i)->m_iid, iid))
				{
					m_aThunks.operator[](i)->InternalAddRef();
					pThunk = m_aThunks.operator[](i);
					break;
				}
			}
		}
	...
	HRESULT AddNonAddRefThunk(IUnknown* p, LPCTSTR lpsz, IUnknown** ppThunkRet)
	{
		...
		*ppThunkRet = NULL;
		...
		for (int i = 0; i < m_aThunks.GetSize(); i++)
		{
			if (m_aThunks.operator[](i)->m_pUnk == p)
			{
				m_aThunks.operator[](i)->m_bNonAddRefThunk = true;
				pThunk = m_aThunks.operator[](i);
				break;
			}
		}
	...
	void DeleteNonAddRefThunk(IUnknown* pUnk)
	{
		...
		for (int i = 0; i < m_aThunks.GetSize(); i++)
		{
			if (m_aThunks.operator[](i)->m_pUnk == pUnk)
			{
				delete m_aThunks.operator[](i);
	...
	void DeleteThunk(_QIThunk* p)
	{
		...
		if (nIndex != -1)
		{
			m_aThunks.operator[](nIndex)->m_pUnk = NULL;
			delete m_aThunks.operator[](nIndex);
...
public:
	...
	CAtlComModule() throw()
	{
		...
		memset(&m_csObjMap, 0, sizeof(CRITICAL_SECTION));
...
public:
	CAtlWinModule()
	{
		...
		if (FAILED(hr))
			RaiseException(hr, EXCEPTION_NONCONTINUABLE, 0, NULL);
	}
...
static GUID m_libid;
...
#ifdef _ATL_CACHE_GIT

	IGlobalInterfaceTable* m_pGIT;

#endif // _ATL_CACHE_GIT
	...
	CAtlModule() throw()
	{
		...
		m_nLockCnt = 0;
		_pAtlModule = this;
		memset(&m_csStaticDataInit, 0, sizeof(CRITICAL_SECTION));
		InitializeCriticalSection(&m_csStaticDataInit);
	...
	void Term() throw()
	{
		...
		DeleteCriticalSection(&m_csStaticDataInit);
		// Call term functions
	...
	virtual HRESULT GetGITPtr(IGlobalInterfaceTable** ppGIT) throw()
	{
		...
		return AtlGetGITPtr(ppGIT);

#endif

	}

	virtual HRESULT AddCommonRGSReplacements(IRegistrar* /*pRegistrar*/) throw()
	{
		return S_OK;
	}
	...
	HRESULT WINAPI UpdateRegistryFromResourceS(LPCTSTR lpszRes, BOOL bRegister,
		struct _ATL_REGMAP_ENTRY* pMapEntries = NULL) throw()
	{
		return UpdateRegistryFromResourceSHelper(lpszRes, bRegister, pMapEntries);
	}
	HRESULT WINAPI UpdateRegistryFromResourceS(UINT nResID, BOOL bRegister,
		struct _ATL_REGMAP_ENTRY* pMapEntries = NULL) throw()
	{
		return UpdateRegistryFromResourceSHelper(MAKEINTRESOURCE(nResID), bRegister, pMapEntries);
	}
		...
		struct _ATL_REGMAP_ENTRY* pMapEntries = NULL) throw()
	{
		...
		if (NULL != pMapEntries)
		{
			while (NULL != pMapEntries->szKey)
			{
				...
				spRegistrar->AddReplacement((LPOLESTR)pMapEntries->szKey, (LPOLESTR)pMapEntries->szData);
				pMapEntries++;
			}
		}
...
#ifdef _ATL_STATIC_REGISTRY
	// Statically linking to Registry Ponent
	HRESULT WINAPI UpdateRegistryFromResourceSHelper(LPCTSTR lpszRes, BOOL bRegister,
		struct _ATL_REGMAP_ENTRY* pMapEntries) throw();
#endif

	static void ReplaceSingleQuote(LPOLESTR lpDest, LPCOLESTR lp) throw()
	{
		while (*lp)
		{
			*lpDest++ = *lp;
...
public :
	...
	{
		HRESULT hr = AtlGetGITPtr(&m_pGIT);
		if (FAILED(hr))
			RaiseException(hr, EXCEPTION_NONCONTINUABLE, 0, NULL);
...
public :
...
#if !defined(_ATL_NO_COM_SUPPORT) || defined(_ATL_CACHE_GIT)

		hr = T::InitializeCom();
		if (FAILED(hr))
			RaiseException(hr, EXCEPTION_NONCONTINUABLE, 0, NULL);

#endif	//  !defined(_ATL_NO_COM_SUPPORT) || defined(_ATL_CACHE_GIT)
	...
	bool StartMonitor() throw()
	{
		m_hEventShutdown = ::CreateEvent(NULL, false, false, NULL);
		if (m_hEventShutdown == NULL)
			return false;
		DWORD dwThreadID;
		CHandle h(::CreateThread(NULL, 0, MonitorProc, this, 0, &dwThreadID));
		return (h != NULL);
	}
	...
	// This is going away. DO NOT use it.
	int WinMain(HINSTANCE hInstance, LPTSTR lpCmdLine, int nShowCmd, LPCTSTR pAppId, 
		_ATL_OBJMAP_ENTRY* pObjectMap = NULL)
	{
		T* pT = static_cast<T*>(this);
		HRESULT hr = pT->Init();
		...
		if (pT->ParseCommandLine(lpCmdLine, pAppId, &hr) == true)
			hr = pT->Run(nShowCmd);
		...
		*pnRetCode = S_OK;
		...
		while (lpszToken != NULL)
		{
			if (lstrcmpi(lpszToken, _T("UnregServer"))==0)
			{
				...
				*pnRetCode = pT->UnregisterAppId(pAppId);
			...
			if (lstrcmpi(lpszToken, _T("RegServer"))==0)
			{
				...
				*pnRetCode = pT->RegisterAppId(pAppId);
				ATLASSERT(SUCCEEDED(*pnRetCode));
				if (FAILED(*pnRetCode))
					return false;
...
public :
	...
	void ServiceMain(DWORD dwArgc, LPTSTR* lpszArgv) throw()
	{
		...
		m_status.dwWin32ExitCode = pT->Run();
	...
	HRESULT PostMessageLoop() throw()
	{
		HRESULT hr = CAtlExeModuleT<T>::PostMessageLoop();
...
public :
	...
	void EnterWindowCriticalSection() throw();
	void LeaveWindowCriticalSection() throw();
...
public:
	...
	HRESULT Attach(T* p) throw()
	{
		...
		if (_pAtlModule == NULL)
			...
			hr = _pAtlModule->GetGITPtr(&spGIT);
		ATLASSERT(spGIT != NULL);
		ATLASSERT(SUCCEEDED(hr));
		if (FAILED(hr))
			return hr;
...
#ifdef _DEBUG
inline CTrace::CTrace(fnCrtDbgReport_t pfnCrtDbgReport) :
	m_hInst(_AtlBaseModule.GetModuleInstance())
{
	ATLVERIFY(AtlTraceRegister(m_hInst, pfnCrtDbgReport));
	RegisterCategory(atlTraceGeneral, _T("atlTraceGeneral"));
	RegisterCategory(atlTraceCOM, _T("atlTraceCOM"));
	RegisterCategory(atlTraceQI, _T("atlTraceQI"));
	RegisterCategory(atlTraceRegistrar, _T("atlTraceRegistrar"));
	RegisterCategory(atlTraceRefcount, _T("atlTraceRefcount"));
	RegisterCategory(atlTraceWindowing, _T("atlTraceWindowing"));
	RegisterCategory(atlTraceControls, _T("atlTraceControls"));
	RegisterCategory(atlTraceHosting, _T("atlTraceHosting"));
	RegisterCategory(atlTraceDBClient, _T("atlTraceDBClient"));
	RegisterCategory(atlTraceDBProvider, _T("atlTraceDBProvider"));
	RegisterCategory(atlTraceSnapin, _T("atlTraceSnapin"));
	RegisterCategory(atlTraceNotImpl, _T("atlTraceNotImpl"));
	RegisterCategory(atlTraceAllocation, _T("atlTraceAllocation"));
	LoadSettings();
}
...
// If used in a DLL change dwWait to something other than INFINITE otherwise
// there will be a deadlock when the DLL is unloaded.
public:
	CAtlAutoThreadModuleT(int nThreads = T::GetDefaultThreads())
	{
		...
		for (int i = 0; i < nThreads; i++)
		{
			...
			m_pApartments[i].m_hThread = (HANDLE)_beginthreadex(NULL, 0, (UINT(WINAPI*)(void*))CComApartment::_Apartment, (void*)&m_pApartments[i], 0, (UINT*)&m_pApartments[i].m_dwThreadID);
			...
			if (m_pApartments[i].m_hThread == NULL)
			{
				RaiseException(AtlHresultFromLastError(), EXCEPTION_NONCONTINUABLE, 0, NULL);
			}
...
// Statically linking to Registry Ponent
inline HRESULT WINAPI CAtlModule::UpdateRegistryFromResourceSHelper(LPCTSTR lpszRes, BOOL bRegister,
	struct _ATL_REGMAP_ENTRY* pMapEntries)
{
	...
	LPOLESTR pszModule;
	if ((_AtlBaseModule.GetModuleInstance() == NULL) || (_AtlBaseModule.GetModuleInstance() == GetModuleHandle(NULL))) // register as EXE
	{
	...
	{
		return (bRegister) ? ro.ResourceRegister(pszModule, (UINT)(UINT_PTR)lpszRes, szType) :
			ro.ResourceUnregister(pszModule, (UINT)(UINT_PTR)lpszRes, szType);
	}
	return (bRegister) ? ro.ResourceRegisterSz(pszModule, T2COLE(lpszRes), szType) :
...
#ifndef UNDER_CE  // REVIEW - What to do with this one
							...
							if ((m_hInst == NULL) || (m_hInst == GetModuleHandle(NULL))) // register as EXE
							{
							   // Convert to short path to work around bug in NT4's CreateProcess
								...
								if (cbShortName == 0 || cbShortName == ERROR_INVALID_PARAMETER )
									...
									lRes = key.SetKeyValue(szLS32, szModuleShort);
							}
							else
							{
...
{
	ATL::CComPtr<IConnectionPointContainer> pCPC;
	ATL::CComPtr<IConnectionPoint> pCP;
	...
	LPCOLESTR lpszHelpFile, const IID& iid, HRESULT hRes, HINSTANCE hInst)
{
	...
	ATL::CComPtr<ICreateErrorInfo> pICEI;
	...
	for (_ATL_OBJMAP_ENTRY** ppEntry = pComModule->m_ppAutoObjMapFirst; ppEntry < pComModule->m_ppAutoObjMapLast; ppEntry++)
	{
		if (*ppEntry != NULL)
		{
			...
			if ((pEntry->pfnGetClassObject != NULL) && InlineIsEqualGUID(rclsid, *pEntry->pclsid))
			{
				if (pEntry->pCF == NULL)
				{
					CCritSecLock lock(pComModule->m_csObjMap);
					if (pEntry->pCF == NULL)
...
ATLINLINE ATLAPI AtlModuleAddTermFunc(_ATL_MODULE* pModule, _ATL_TERMFUNC* pFunc, DWORD_PTR dw)
{
	...
	else
	{
		pNew->pFunc = pFunc;
		pNew->dw = dw;
		CCritSecLock lock(pModule->m_csStaticDataInit);
...
{
   ATL::CComPtr< ICatRegister > pCatRegister;
   HRESULT hResult;
	...
	BOOL bRegister, struct _ATL_REGMAP_ENTRY* pMapEntries, IRegistrar* pReg)
{
	...
	if (SUCCEEDED(hRes))
	{
		...
		ATL::CAtlModule::ReplaceSingleQuote(pszModuleQuote, pszModule);
...
ATLINLINE ATLAPI AtlLoadTypeLib(HINSTANCE hInstTypeLib, LPCOLESTR lpszIndex, BSTR* pbstrPath, ITypeLib** ppTypeLib)
{
	...
	*ppTypeLib = NULL;
	...
	// get the extention pointer in case of fail
	LPTSTR lpszExt = NULL;
	LPTSTR lpsz;
...
ATLINLINE ATLAPI_(void) AtlWinModuleAddCreateWndData(_ATL_WIN_MODULE* pWinModule, _AtlCreateWndData* pData, void* pObject)
{
	...
	CCritSecLock lock(pWinModule->m_csWindowCreate);
...
inline HRESULT __AtlWinModuleCSInit(_ATL_WIN_MODULE* pWinModule)
{
	__try
	{
		memset(&pWinModule->m_csWindowCreate, 0, sizeof(CRITICAL_SECTION));
		InitializeCriticalSection(&pWinModule->m_csWindowCreate);
	}
	// structured exception may be raised in low memory situations
	__except(EXCEPTION_EXECUTE_HANDLER)
	{
...
ATLINLINE ATLAPI AtlWinModuleInit(_ATL_WIN_MODULE* pWinModule)
{
	...
	HRESULT hr = __AtlWinModuleCSInit(pWinModule);
...
ATLINLINE ATLAPI AtlWinModuleTerm(_ATL_WIN_MODULE* pWinModule, HINSTANCE hInst)
{
	...
	DeleteCriticalSection(&pWinModule->m_csWindowCreate);
...
#else
	DLLGETVERSIONPROC pfnDllGetVersion = (DLLGETVERSIONPROC)::GetProcAddress(hInstDLL, L"DllGetVersion");
#endif
