...
//ATLAPI_(ATOM) AtlModuleRegisterClassExA(_ATL_MODULE* pM, const WNDCLASSEXA *lpwc);
...
//ATLAPI_(ATOM) AtlModuleRegisterClassExW(_ATL_MODULE* pM, const WNDCLASSEXW *lpwc);
...
#pragma pack(pop)
	...
	static HRESULT ParseInitData(IStream* pStream, BSTR* pLicKey)
	{
		*pLicKey = NULL;
		...
		if (nMsg == ATL_WM_OCC_LOADFROMSTREAM_EX ||
			nMsg == ATL_WM_OCC_LOADFROMSTORAGE_EX)
		{
		...
		if (nMsg == ATL_WM_OCC_LOADFROMSTREAM)
		{
			return S_OK;
		}
		return E_FAIL;
...
//N new
	...
	BOOL ResizeClient(int nWidth, int nHeight, BOOL bRedraw = TRUE) throw()
	{
		...
		if(!::AdjustWindowRectEx(&rcWnd, GetStyle(), (!(GetStyle() & WS_CHILD) && (GetMenu() != NULL)), GetExStyle()))
			return FALSE;
#else
		if(!::AdjustWindowRectEx(&rcWnd, GetStyle(), (!(GetStyle() & WS_CHILD)), GetExStyle()))
			return FALSE;
#endif // UNDER_CE
...
#else
typedef CWinTraits<WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 0>					CControlWinTraits;
typedef CWinTraits<WS_CLIPCHILDREN | WS_CLIPSIBLINGS, WS_EX_WINDOWEDGE>		CFrameWinTraits;
#endif
...
#ifndef UNDER_CE // MulDiv not present on CE
