...
#define MAXOPENROWS_Flags ( DBPROPFLAGS_ROWSET | DBPROPFLAGS_READ | DBPROPFLAGS_WRITE ) 
...
#define PROVIDEROLEDBVER_Value OLESTR("02.00") 
		...
		for(ulSet=0; ulSet<ulEnd; ulSet++)
		{
			...
			if (cPropertySets == 0)
			{
				if (pGuid != NULL)
				{
					...
					for (ULONG ulCurrentInitSet = ulSet; ulCurrentInitSet < m_cUPropSet; ulCurrentInitSet++)
					{
...
public:
	...
	STDMETHOD(GetSchemas)(ULONG * pcSchemas, GUID ** prgSchemas, ULONG** prgRest)
	{
		...
		if (prgSchemas != NULL)
			*prgSchemas = NULL;
		if (pcSchemas == NULL || prgSchemas == NULL || prgRest == NULL) 
			return E_INVALIDARG;
...
public:
				...
				DBBINDSTATUS rgStatus[], bool bHasBookmarks)
	{
		...
		for (ULONG iBinding = 0; iBinding < cBindings; iBinding++)
		{
			...
			// In OLE DB 2.5 only the DBBINDFLAG_HTML flag is supported in the 
			// dwFlags parameter, all others will return an error.
...
//				if(!pT->m_rgRowData.RemoveAt(pRow->m_iRowset))
				...
				else
				{
					...
					{	
						...
						{
							if (pT->m_rgBookmarks[k] == (pRow->m_iRowset + 1))
			...
			for (DBORDINAL i = 0;i < cCols; i++)
			{
				if (pInfo[i].iOrdinal == 0)
				{
					...
					case DBTYPE_BYTES: //review: check the line below (used to be DWORD instead of DBBKMARK)
						*((DBBKMARK*)(&newRow + pInfo[i].cbOffset)) = pT->m_rgRowData.GetCount() + 1;
						break;
					default:
						ATLASSERT(FALSE);
					};
...
//			lSize = 0;		// on the rare occasion we got a negative back
		...
		hr = pT->CreateRow(lSize, ulRowsObtained, &hInsertedRow);
...
public:
					...
					DBROWSTATUS rgRowStatus[], BOOL bAdd)
	{
		...
		__if_exists(T::Fire_OnRowsetChange)
		{
			...
			{
				T* pT = (T*)this;
				pT->Fire_OnRowChange(pT, arrZeroHandles.GetCount(), arrZeroHandles.GetData(), 
...
// REVIEW:		rpSrcData = (void*)&(pT->m_rgRowData[pRow->m_iRowset]);
						   ...
						   HROW **prghRows)
	{
		...
		*pcRowsObtained = 0;
		...
		if (*prghRows == NULL)
		{
			DBROWOFFSET cHandlesToAlloc = __min(cRowsInSet, cRows); //(cRows > cRowsInSet) ? cRowsInSet : cRows;
...
public:
		...
		DBROWCOUNT cRows, DBCOUNTITEM * pcRowsObtained, HROW ** prghRows)
	{
		...
		*pcRowsObtained = 0;
		...
		if ((cbBookmark != 1) && (m_rgBookmarks[lBookmarkIndex] == -1))
		{
...
//				lRow = ((long)*rgpBookmarks[l]) - 1;
	...
	HRESULT ValidateBookmark(DBBKMARK cbBookmark, const BYTE* pBookmark)
	{
		...
		{
			ATLTRACE(atlTraceDBProvider, 2, _T("Bookmarks are invalid length, should be DWORDs"));
			return DB_E_BADBOOKMARK;
		}

		// If the contents of our bookmarks are less than 0 or greater than
		// rowcount, then they are invalid
		DBCOUNTITEM nRows = pT->m_rgBookmarks.GetCount();
		...
		if (((lBookmarkIndex <= -1 || lBookmarkIndex > (DBROWOFFSET)nRows)
			...
			(cbBookmark == 1 && lBookmarkIndex == 0 /* DBBMK_INVALID */))
		{
...
class IFakeRowsetNotifyCP
{
public:
	HRESULT Fire_OnFieldChange(IRowset*, HROW, ULONG, ULONG*, DBREASON, DBEVENTPHASE, BOOL)
	{
		return S_OK;
	}
	HRESULT Fire_OnRowChange(IRowset*, ULONG, HROW*, DBREASON, DBEVENTPHASE, BOOL)
	{
		return S_OK;
	}
	HRESULT Fire_OnRowsetChange(IRowset*, DBREASON, DBEVENTPHASE, BOOL)
	{
		return S_OK;
	}
};
...
//				{
		...
		DBROWSTATUS **prgRowStatus)
	{
		...
		for (ULONG ulRow = 0; ulRow < ulRowUpdate; ulRow++)
		{
			...
			// Send preliminary notifications before beginning processing
			__if_exists(T::Fire_OnRowChange)
			{
