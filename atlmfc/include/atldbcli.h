...
#define DEFINE_OLEDB_TYPE_FUNCTION(ctype, oledbtype) \
	...
	//@todo - review the two lines below
...
{
public:
	static HRESULT GetDefaultCommand(LPCTSTR* /*ppszCommand*/) throw ()
	{
		return S_OK;
	}
};
...
public:
	...
	HRESULT MoveNext(DBROWOFFSET lSkip, bool bForward = true) throw()
	{
		...
		DBCOUNTITEM nNewRow = m_nCurrentRow + lSkip + (bForward ? 1 : -1);
		...
		else if (nNewRow >= m_nCurrentRows)
		{
			bFetchNewRows = true;
...
//	void FreeRecordMemory(int , IRowset* ) throw() {}
	...
	bool GetLength(DBORDINAL nColumn, DBLENGTH* pLength) const throw()
	{
		...
		if (TranslateColumnNo(nColumn))
		{
			...
			IncrementAndAlignOffset( nOffset, m_pColumnInfo[nColumn].ulColumnSize, alignof(DBLENGTH) );
	...
	bool GetStatus(DBORDINAL nColumn, DBSTATUS* pStatus) const throw()
	{
		...
		if (TranslateColumnNo(nColumn))
		{
			DBBYTEOFFSET nOffset = (DBBYTEOFFSET)(ULONG_PTR)m_pColumnInfo[nColumn].pTypeInfo;
			IncrementAndAlignOffset( nOffset, m_pColumnInfo[nColumn].ulColumnSize, alignof(DBLENGTH) );
			IncrementAndAlignOffset( nOffset, sizeof(DBLENGTH), alignof(DBSTATUS) );
	...
	HRESULT GetColumnInfo(IRowset* pRowset, DBORDINAL* pColumns, DBCOLUMNINFO** ppColumnInfo) throw()
	{
		CComPtr<IColumnsInfo> spColumnsInfo;
...
// Implementation
	...
	void FreeRecordMemory(IRowset* pRowset) throw()
	{
		...
		for (i = 0; i < m_nColumns; i++)
		{
			...
			{
				//@todo: check this out - it seems that at least some providers do not set the reference pointer
				//       (to value) to null when they report DBSTATUS_S_ISNULL for a given column
				DBBYTEOFFSET nOffset = (DBBYTEOFFSET)(ULONG_PTR)m_pColumnInfo[i].pTypeInfo;
				IncrementAndAlignOffset( nOffset, m_pColumnInfo[i].ulColumnSize, alignof(DBLENGTH) );
				IncrementAndAlignOffset( nOffset, sizeof(DBLENGTH), alignof(DBSTATUS) );
		...
		for (i = 0; i < m_nColumns; i++)
		{
			...
			if (m_pColumnInfo[i].ulColumnSize > m_nBlobSize && m_pColumnInfo[i].wType != DBTYPE_IUNKNOWN )
			{
				if( m_eBlobHandling == DBBLOBHANDLING_SKIP )
				{
					...
					nLengthOffset = AlignAndIncrementOffset( nOffset, sizeof(DBLENGTH), alignof(DBLENGTH) );
					// Calculate the column status offset
					nStatusOffset = AlignAndIncrementOffset( nOffset, sizeof(DBSTATUS), alignof(DBSTATUS) );
	...
	static size_t GetAlignment(DBTYPE bType) throw()
	{
		...
		if( bType & DBTYPE_ARRAY )
			return alignof(SAFEARRAY*);
		
		if( bType & DBTYPE_VECTOR )
			return alignof(DBVECTOR);
		...
		case DBTYPE_IDISPATCH:
			return alignof(IDispatch*);
			break;

		case DBTYPE_ERROR:
			return alignof(SCODE);
			break;

		case DBTYPE_BOOL:
			return alignof(VARIANT_BOOL);
			break;

		case DBTYPE_VARIANT:
			return alignof(VARIANT);
			break;

		case DBTYPE_IUNKNOWN:
			return alignof(IUnknown*);
			break;

		case DBTYPE_DECIMAL:
			return alignof(DECIMAL);
			break;
		...
		case DBTYPE_NUMERIC:
			return alignof(DB_NUMERIC);
			break;

		case DBTYPE_DBDATE:
			return alignof(DBDATE);
			break;

		case DBTYPE_DBTIME:
			return alignof(DBTIME);
			break;

		case DBTYPE_DBTIMESTAMP:
			return alignof(DBTIMESTAMP);
			break;
	...
	inline static DBBYTEOFFSET AlignAndIncrementOffset( DBBYTEOFFSET& nOffset, DBLENGTH nSize, size_t nAlign ) throw()
	{
		...
		nOffset = AtlAlignUp( nOffset, nAlign );
		nResult = nOffset;
		nOffset += nSize;
...
public:
	HRESULT BindColumns(IUnknown* pUnk) throw()
	{
		...
		for (i = 0; i < m_nColumns; i++)
		{
			...
			// Calculate the column data offset
			nDataOffset = AlignAndIncrementOffset(nOffset, sizeof(VARIANT), alignof(VARIANT));
...
public :
	CDataConnectionHelper() throw()
	{
	}

	CDataConnectionHelper(const CDataConnectionHelper &ds) throw()
	{
		Copy(ds);
	}

	CDataConnectionHelper& operator=(const CDataConnectionHelper &ds) throw()
	{
		return Copy(ds);
	}

	CDataConnectionHelper& Copy(const CDataConnectionHelper &ds) throw()
	{
    ...
    HRESULT OpenDataSource(LPCOLESTR szInitializationString) throw()
    {
        HRESULT hr;
        hr = m_source.OpenFromInitializationString(szInitializationString);
        if (FAILED(hr))
		{
#ifdef _DEBUG
        ...
        return m_session.Open(m_source);
    }
...
class CDataConnection : public CDataSource
{
public:
	CDataConnection()
	{
	...
	{
		m_spInit = ds.m_spInit;
		m_session.m_spOpenRowset = ds.m_session.m_spOpenRowset;
		return *this;
	}

	HRESULT Open(LPCTSTR szInitString) throw()
	{
		HRESULT hr = E_FAIL;
		CT2W szinit(szInitString);
		hr = OpenFromInitializationString(szinit);
		if (hr == S_OK)
		{
			hr = m_session.Open(*this);
		}
		return hr;
	}
...
// Implementation
	...
	{
		/*
		@todo: review the code below - this assert below should not be necessary. The function will work fine
		       even if ppRowset is NULL.  And a later assert (in Bind) will catch it (in most cases).
			   Before we can try to access the data in the rowset we have to call Bind, and Bind will assert
			   if the rowset pointer is NULL

		if( typeid(TRowset<TAccessor >) == typeid(CNoRowset<TAccessor >) )
		{
			ATLASSERT(ppRowset != NULL);
			if (ppRowset == NULL)
				return E_POINTER;
			*ppRowset = NULL;
		}
