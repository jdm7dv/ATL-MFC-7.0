...
public:
	...
	const SID_IDENTIFIER_AUTHORITY *GetPSID_IDENTIFIER_AUTHORITY() const
		{return ::GetSidIdentifierAuthority(m_pSid);}
	DWORD GetSubAuthority(DWORD nSubAuthority) const
		{return *::GetSidSubAuthority(m_pSid, nSubAuthority);}
...
private:
	...
	// REVIEW: I use a lot of "logical const" member variables
	// in this file - gutcheck.
				  ...
				  : m_pSid(NULL), m_SidNameUse(SidTypeInvalid)
{
	...
	pSid = (SID *) _alloca(::GetSidLengthRequired(nSubAuthorityCount));
	...
	for(unsigned i = 0; i < nSubAuthorityCount; i++)
...
inline bool CSid::LoadAccount(LPCTSTR pszAccountName, LPCTSTR pszSystem)
{
	...
	static const DWORD dwDomainSize = 32; // seems reasonable
	...
	{
		// LookupAccountName doesn't change cbSid on success (although it changes cbDomain - grrr...)
		if(bSuccess)
			cbSid = ::GetLengthSid(pSid);

		free(m_pSid);
		m_pSid = (SID *) malloc(cbSid);
		ATLASSERT(m_pSid);
		if(!m_pSid)
		...
		else
		{
			...
			{
				LPTSTR pszDomain = (LPTSTR) m_strDomain.GetBuffer(cbDomain);
...
inline LPCTSTR CSid::Sid() const
{
	if(m_strSid.IsEmpty())
	{
		...
		unsigned nSubAuthorityCount = *::GetSidSubAuthorityCount(m_pSid);
		CString strTemp;
...
inline void CSid::GetAccountNameAndDomain() const
{
	...
	else
	{
		...
		case ERROR_INSUFFICIENT_BUFFER:
		{
			...
			::LookupAccountSid(m_strSystem, m_pSid, pszName, &cbName, pszDomain, &cbDomain, &m_SidNameUse);
		...
		case ERROR_NONE_MAPPED:
			// REVIEW: Global strings to RC file
			m_strAccountName.Empty();
			m_strDomain.Empty();
			m_SidNameUse = SidTypeUnknown;
			break;
...
template<>
class CElementTraitsBase< CSid >
{
public:
	typedef const CSid& INARGTYPE;
	typedef CSid& OUTARGTYPE;
	...
	static void CopyElements( CSid* pDest, const CSid* pSrc, size_t nElements )
	{
...
public:
	...
	virtual unsigned GetAceCount() const = 0;
...
protected:
	...
	virtual const CAce *GetAce(unsigned nIndex) const = 0;
	virtual void RemoveAce(unsigned nIndex) = 0;
...
inline const ACL *CAcl::GetPACL() const
{
	if(!m_pAcl && !m_bNull)
	{
		unsigned nAclLength = sizeof(ACL);
		const CAce *pAce;
		unsigned i;
		const unsigned nCount = GetAceCount();
...
inline unsigned CAcl::GetLength() const
{
...
private:
	...
	public:
		...
		BYTE AceType() const
			{return m_bAllow ? ACCESS_ALLOWED_ACE_TYPE : ACCESS_DENIED_ACE_TYPE;}
	...
	public:
		...
		unsigned GetLength() const;
		BYTE AceType() const
			{return m_bAllow ? ACCESS_ALLOWED_OBJECT_ACE_TYPE : ACCESS_DENIED_OBJECT_ACE_TYPE;}
...
#endif
	const CAcl::CAce *GetAce(unsigned nIndex) const
		{return m_Acl[nIndex];}
	void RemoveAce(unsigned nIndex);
...
inline void CDacl::Copy(const ACL &rhs)
{
	...
	for(DWORD i = 0; i < AclSizeInfo.AceCount; i++)
	{
		...
		case ACCESS_DENIED_ACE_TYPE:
			...
			pAce = new CAccessAce(Sid, AccessMask, pHeader->AceFlags,
		...
		case ACCESS_DENIED_OBJECT_ACE_TYPE:
		{
			...
			pAce = new CAccessObjectAce(Sid, AccessMask, pHeader->AceFlags,
...
inline bool CDacl::AddAllowedAce(const CSid &rSid, ACCESS_MASK AccessMask, BYTE AceFlags)
{
	...
	CAccessAce *pAce = new CAccessAce(rSid, AccessMask, AceFlags, true);
	ATLASSERT(pAce);
	if(!pAce)
		return false;
								 ...
								 const GUID *pObjectType, const GUID *pInheritedObjectType)
{
	...
	CAccessAce *pAce = new CAccessObjectAce(rSid, AccessMask, AceFlags, true,
		pObjectType, pInheritedObjectType);
	ATLASSERT(pAce);
	if(!pAce)
		return false;
...
inline bool CDacl::AddDeniedAce(const CSid &rSid, ACCESS_MASK AccessMask, BYTE AceFlags)
{
	...
	CAccessAce *pAce = new CAccessAce(rSid, AccessMask, AceFlags, false);
	ATLASSERT(pAce);
	if(!pAce)
		return false;
								...
								const GUID *pObjectType, const GUID *pInheritedObjectType)
{
	...
	CAccessAce *pAce = new CAccessObjectAce(rSid, AccessMask, AceFlags, false,
		pObjectType, pInheritedObjectType);
	ATLASSERT(pAce);
	if(!pAce)
		return false;
...
inline void CDacl::RemoveAce(unsigned nIndex)
{
...
inline void *CDacl::CAccessAce::GetACE() const
{
	...
	{
	int
	Allowed_Header		= offsetof(ACCESS_ALLOWED_ACE, Header),
	Allowed_Mask		= offsetof(ACCESS_ALLOWED_ACE, Mask),
	Allowed_SidStart	= offsetof(ACCESS_ALLOWED_ACE, SidStart),
	Denied_Header		= offsetof(ACCESS_DENIED_ACE, Header),
	Denied_Mask			= offsetof(ACCESS_DENIED_ACE, Mask),
	Denied_SidStart		= offsetof(ACCESS_DENIED_ACE, SidStart);

	ATLASSERT(sizeof(ACCESS_ALLOWED_ACE) == sizeof(ACCESS_DENIED_ACE));
	ATLASSERT(Allowed_Header == Denied_Header);
	ATLASSERT(Allowed_Mask == Denied_Mask);
	ATLASSERT(Allowed_SidStart == Denied_SidStart);
	}
#endif

	if(!m_pAce)
	{
		unsigned nLength = GetLength();
		ACCESS_ALLOWED_ACE *pAce = (ACCESS_ALLOWED_ACE *) malloc(nLength);
		ATLASSERT(pAce);
		if(!pAce)
			return NULL;

		::ZeroMemory(pAce, nLength);
	...
	m_pInheritedObjectType(NULL)
{
	if(pObjectType)
	{
		m_pObjectType = new GUID(*pObjectType);
		if(!m_pObjectType)
			AtlThrow(E_OUTOFMEMORY);
	}
	...
	{
		m_pInheritedObjectType = new GUID(*pInheritedObjectType);
...
inline CDacl::CAccessObjectAce &CDacl::CAccessObjectAce::operator=(const CAccessObjectAce &rhs)
{
	if(this != &rhs)
	{
		...
		if(rhs.m_pInheritedObjectType)
		{
			if(!m_pInheritedObjectType)
			{
				m_pInheritedObjectType = new GUID;
				if(!m_pInheritedObjectType)
				{
					delete m_pObjectType;
					m_pObjectType = NULL;
...
inline unsigned CDacl::CAccessObjectAce::GetLength() const
{
	unsigned nLength = offsetof(ACCESS_ALLOWED_OBJECT_ACE, SidStart);
...
inline void *CDacl::CAccessObjectAce::GetACE() const
{
	...
	{
	int
	Allowed_Header						= offsetof(ACCESS_ALLOWED_OBJECT_ACE, Header),
	Allowed_Mask						= offsetof(ACCESS_ALLOWED_OBJECT_ACE, Mask),
	Allowed_Flags						= offsetof(ACCESS_ALLOWED_OBJECT_ACE, Flags),
	Allowed_ObjectType					= offsetof(ACCESS_ALLOWED_OBJECT_ACE, ObjectType),
	Allowed_InheritedObjectType			= offsetof(ACCESS_ALLOWED_OBJECT_ACE, InheritedObjectType),
	Allowed_SidStart					= offsetof(ACCESS_ALLOWED_OBJECT_ACE, SidStart),
	Denied_Header						= offsetof(ACCESS_DENIED_OBJECT_ACE, Header),
	Denied_Mask							= offsetof(ACCESS_DENIED_OBJECT_ACE, Mask),
	Denied_Flags						= offsetof(ACCESS_DENIED_OBJECT_ACE, Flags),
	Denied_ObjectType					= offsetof(ACCESS_DENIED_OBJECT_ACE, ObjectType),
	Denied_InheritedObjectType			= offsetof(ACCESS_DENIED_OBJECT_ACE, InheritedObjectType),
	Denied_SidStart						= offsetof(ACCESS_DENIED_OBJECT_ACE, SidStart);

	ATLASSERT(sizeof(ACCESS_ALLOWED_OBJECT_ACE) == sizeof(ACCESS_DENIED_OBJECT_ACE));
	ATLASSERT(Allowed_Header == Denied_Header);
	ATLASSERT(Allowed_Mask == Denied_Mask);
	ATLASSERT(Allowed_Flags == Denied_Flags);
	ATLASSERT(Allowed_ObjectType == Denied_ObjectType);
	ATLASSERT(Allowed_InheritedObjectType == Denied_InheritedObjectType);
	ATLASSERT(Allowed_SidStart == Denied_SidStart);
	}
#endif
	...
	{
		unsigned nLength = GetLength();
		ACCESS_ALLOWED_OBJECT_ACE *pAce = (ACCESS_ALLOWED_OBJECT_ACE *) malloc(nLength);
		ATLASSERT(pAce);
		if(!pAce)
			return NULL;
		...
		BYTE *pb = ((BYTE *) pAce) + offsetof(ACCESS_ALLOWED_OBJECT_ACE, SidStart);
		if(!m_pObjectType)
			pb -= sizeof(GUID);
		else
		{
...
private:
	...
	const CAce *GetAce(unsigned nIndex) const
		{return m_Acl[nIndex];}
	void RemoveAce(unsigned nIndex);
...
inline void CSacl::Copy(const ACL &rhs)
{
	...
	for(DWORD i = 0; i < AclSizeInfo.AceCount; i++)
	{
		...
		case SYSTEM_AUDIT_ACE_TYPE:
			Sid = *reinterpret_cast<SID *>
				(reinterpret_cast<BYTE *>(pHeader) +	sizeof(ACE_HEADER) + sizeof(ACCESS_MASK));
			pAce = new CAuditAce(Sid, AccessMask, pHeader->AceFlags, bSuccess, bFailure);
			ATLASSERT(pAce);
			if(pAce)
				m_Acl.Add(pAce);
			break;
		...
		case SYSTEM_AUDIT_OBJECT_ACE_TYPE:
		{
			...
			pAce = new CAuditObjectAce(Sid, AccessMask, pHeader->AceFlags,
							   ...
							   bool bSuccess, bool bFailure, BYTE AceFlags)
{
	...
	CAuditAce *pAce = new CAuditAce(rSid, AccessMask, AceFlags, bSuccess, bFailure);
	ATLASSERT(pAce);
	if(!pAce)
		return false;
							   ...
							   const GUID *pObjectType, const GUID *pInheritedObjectType)
{
	...
	CAuditAce *pAce = new CAuditObjectAce(rSid, AccessMask, AceFlags, bSuccess,
...
inline void CSacl::RemoveAce(unsigned nIndex)
{
	delete GetAce(nIndex);
	m_Acl.RemoveAt(nIndex);
}
...
inline void *CSacl::CAuditAce::GetACE() const
{
	...
	{
		unsigned nLength = GetLength();
		SYSTEM_AUDIT_ACE *pAce = (SYSTEM_AUDIT_ACE *) malloc(nLength);
		ATLASSERT(pAce);
		if(!pAce)
			return NULL;
...
inline unsigned CSacl::CAuditObjectAce::GetLength() const
{
	unsigned nLength = offsetof(SYSTEM_AUDIT_OBJECT_ACE, SidStart);
...
inline void *CSacl::CAuditObjectAce::GetACE() const
{
	...
	{
		unsigned nLength = GetLength();
		SYSTEM_AUDIT_OBJECT_ACE *pAce = (SYSTEM_AUDIT_OBJECT_ACE *) malloc(nLength);
		ATLASSERT(pAce);
		if(!pAce)
			return NULL;
...
inline void CSecurityDesc::Init(const SECURITY_DESCRIPTOR &rhs)
{
	...
	m_pSecurityDescriptor = (SECURITY_DESCRIPTOR *) malloc(dwLen);
...
inline bool CSecurityDesc::MakeSelfRelative()
{
	...
	pSD = (SECURITY_DESCRIPTOR *) malloc(dwLen);
	ATLASSERT(pSD);
	if(!pSD)
		return false;
...
{
	// m_pSecurityDescriptor should never NOT be NULL.
	ATLASSERT(!m_pSecurityDescriptor);
	// just in case though...
	free(m_pSecurityDescriptor);

	m_pSecurityDescriptor = (SECURITY_DESCRIPTOR *) malloc(sizeof(SECURITY_DESCRIPTOR));
	ATLASSERT(m_pSecurityDescriptor);
	if(!m_pSecurityDescriptor)
		return false;
...
inline bool CSecurityDesc::SetOwner(const CSid &Sid, bool bDefaulted)
{
	...
	unsigned nSidLength = Sid.GetLength();
...
#ifdef _DEBUG
	if(bPresent)
		ATLTRACE(atlTraceSecurity, 2, "Caution: Setting Dacl to Null\n");
#endif
...
inline bool CSecurityDesc::SetDacl(const CDacl &Dacl, bool bDefaulted)
{
	...
	{
		unsigned nAclLength = Dacl.GetLength();
		if(!nAclLength)
			return false;

		pNewDacl = (ACL *) malloc(nAclLength);
		ATLASSERT(pNewDacl);
		if(!pNewDacl)
			return false;
...
inline bool CSecurityDesc::SetSacl(const CSacl &Sacl, bool bDefaulted)
{
	...
	{
		unsigned nAclLength = Sacl.GetLength();
		if(!nAclLength)
			return false;

		pNewSacl = (ACL *) malloc(nAclLength);
		ATLASSERT(pNewSacl);
		if(!pNewSacl)
			return false;
...
public:
	...
	static void CopyElements( LUID* pDest, const LUID* pSrc, size_t nElements )
	{
		for( size_t iElement = 0; iElement < nElements; iElement++ )
			pDest[iElement] = pSrc[iElement];
	}

	static void RelocateElements( LUID* pDest, LUID* pSrc, size_t nElements )
	{
		// A simple memmove works for nearly all types.
		// You'll have to override this for types that have pointers to their
		// own members.
		memmove( pDest, pSrc, nElements*sizeof( LUID ) );
	}
...
public:
	...
	CTokenPrivileges(const CTokenPrivileges &rhs) :
		m_TokenPrivileges(rhs.m_TokenPrivileges), m_pTokenPrivileges(NULL), m_bDirty(true){}
	CTokenPrivileges &operator=(const CTokenPrivileges &rhs)
		{if(this != &rhs){m_TokenPrivileges = rhs.m_TokenPrivileges; m_bDirty = true;} return *this;}
	...
	unsigned GetSize() const {return (unsigned) m_TokenPrivileges.GetCount();}

	unsigned GetLength() const
		{return offsetof(TOKEN_PRIVILEGES, Privileges) + sizeof(LUID_AND_ATTRIBUTES) * GetSize();}
...
inline const TOKEN_PRIVILEGES *CTokenPrivileges::GetPTOKEN_PRIVILEGES() const
{
	if(m_bDirty)
	{
		...
		{
			m_pTokenPrivileges = (TOKEN_PRIVILEGES *) malloc(GetLength());
			ATLASSERT(m_pTokenPrivileges);
			if(!m_pTokenPrivileges)
				return NULL;

			m_pTokenPrivileges->PrivilegeCount = (DWORD) GetSize();
...
{
	m_bDirty = true;
	for(unsigned i = 0; i < rPrivileges.PrivilegeCount; i++)
...
inline void CTokenPrivileges::GetNamesAndAttributes(CSimpleArray<CString> *pNames,
...
inline void CTokenPrivileges::GetDisplayNames(CSimpleArray<CString> *pDisplayNames) const
{
...
public:
	...
	CTokenGroups(const CTokenGroups &rhs) :
		m_TokenGroups(rhs.m_TokenGroups), m_pTokenGroups(NULL), m_bDirty(true){}
	CTokenGroups &operator=(const CTokenGroups &rhs)
		{if(this != &rhs){m_TokenGroups = rhs.m_TokenGroups; m_bDirty = true;} return *this;}
	...
	unsigned GetSize() const {return (unsigned) m_TokenGroups.GetCount();}

	unsigned GetLength() const
		{return unsigned(offsetof(TOKEN_GROUPS, Groups) +
...
private:
	class CTGElementTraits : public CElementTraitsBase<CSid>
	{
	public:
...
inline const TOKEN_GROUPS *CTokenGroups::GetPTOKEN_GROUPS() const
{
	if(m_bDirty)
	{
		...
		{
			m_pTokenGroups = (TOKEN_GROUPS *) malloc(GetLength());
			ATLASSERT(m_pTokenGroups);
			if(!m_pTokenGroups)
				return NULL;
...
{
	m_bDirty = true;
	for(unsigned i = 0; i < rTokenGroups.GroupCount; i++)
		m_TokenGroups.SetAt(
...
public:
	...
	bool CreateRestrictedToken(CAccessToken *pRestrictedToken,
		const CTokenGroups &SidsToDisable, const CTokenPrivileges &PrivilegesToDisable,
		const CTokenGroups &SidsToRestrict) const;
	bool CreateRestrictedToken(CAccessToken *pRestrictedToken,
		const CTokenGroups &SidsToDisable, const CTokenGroups &SidsToRestrict) const;
#endif
												  ...
												  CTokenPrivileges *pPreviousState)
{
	...
	if(pPreviousState)
	{
		DWORD dwLength = offsetof(TOKEN_PRIVILEGES, Privileges) +
			rNewState.GetSize() * sizeof(LUID_AND_ATTRIBUTES);

		TOKEN_PRIVILEGES *pPrevState = (TOKEN_PRIVILEGES *) _alloca(dwLength);
											 ...
											 bool bImpersonate, bool bOpenAsSelf)
{
	...
	{
		m_pRevert = new CRpcRevertToSelfEx(BindingHandle);
		ATLASSERT(m_pRevert);
		if(!m_pRevert)
		{
			::RpcRevertToSelfEx(BindingHandle);
												...
												const CTokenGroups &SidsToRestrict) const
{
	...
	HANDLE hToken;
	SID_AND_ATTRIBUTES *pSidsToDisable, *pSidsToRestrict;
	LUID_AND_ATTRIBUTES *pPrivilegesToDelete;

	pSidsToDisable = const_cast<SID_AND_ATTRIBUTES *>
		(SidsToDisable.GetPTOKEN_GROUPS()->Groups);
	pSidsToRestrict = const_cast<SID_AND_ATTRIBUTES *>
		(SidsToRestrict.GetPTOKEN_GROUPS()->Groups);
	pPrivilegesToDelete = const_cast<LUID_AND_ATTRIBUTES *>
		(PrivilegesToDelete.GetPTOKEN_PRIVILEGES()->Privileges);

	if(!::CreateRestrictedToken(m_hToken, 0,
		SidsToDisable.GetSize(), pSidsToDisable,
		PrivilegesToDelete.GetSize(), pPrivilegesToDelete,
		SidsToRestrict.GetSize(), pSidsToRestrict, &hToken))
	{
		return false;
	}
												...
												const CTokenGroups &SidsToRestrict) const
{
	...
	if(!::CreateRestrictedToken(m_hToken, DISABLE_MAX_PRIVILEGE,
		SidsToDisable.GetSize(), pSidsToDisable,
		0, NULL,
		SidsToRestrict.GetSize(), pSidsToRestrict, &hToken))
	{
...
inline bool CAccessToken::GetLogonSid(CSid *pSid) const
{
	...
	TOKEN_GROUPS *pGroups = (TOKEN_GROUPS *) _alloca(dwLen);
...
inline void CPrivateObjectSecurityDesc::Clear()
{
	...
	{
#ifdef _DEBUG
		ATLASSERT(::DestroyPrivateObjectSecurity(
			reinterpret_cast<PSECURITY_DESCRIPTOR *>(&m_pSecurityDescriptor)));
#else
		::DestroyPrivateObjectSecurity(reinterpret_cast<PSECURITY_DESCRIPTOR *>(&m_pSecurityDescriptor));
#endif
		m_bPrivate = false;
		m_pSecurityDescriptor = NULL;
	}
...
inline bool AtlGetOwnerSid(LPCTSTR pszObjectName, SE_OBJECT_TYPE ObjectType, CSid *pSid)
{
	...
	DWORD dwErr = ::GetNamedSecurityInfo(const_cast<LPTSTR>(pszObjectName), ObjectType,
		OWNER_SECURITY_INFORMATION, (PSID *) &pOwner, NULL, NULL, NULL, &pSD);
...
inline bool AtlGetGroupSid(HANDLE hObject, SE_OBJECT_TYPE ObjectType, CSid *pSid)
{
	...
	DWORD dwErr = ::GetSecurityInfo(hObject, ObjectType, GROUP_SECURITY_INFORMATION,
		NULL, (PSID *) &pGroup, NULL, NULL, &pSD);
