...
public:
	...
	{
		// Always called from CComCreator2::CreateInstance
		return hr;
	}
};
...
#define OBJECT_ENTRY_AUTO(clsid, class) \
	...
	extern "C" __declspec(allocate("ATL$__m")) __declspec(selectany) ATL::_ATL_OBJMAP_ENTRY* __pobjMap_##class = &__objMap_##class; \
...
inline HRESULT CComTypeInfoHolder::GetTI(LCID lcid)
{
	...
	if (m_pInfo != NULL && m_pMap != NULL)
		return S_OK;
	HRESULT hRes = E_FAIL;
	_pAtlModule->EnterStaticDataCriticalSection();
	if (m_pInfo == NULL)
	{
		ITypeLib* pTypeLib;
	...
	_pAtlModule->LeaveStaticDataCriticalSection();
	return hRes;
}
