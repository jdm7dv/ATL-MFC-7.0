...
namespace ATL
{
// REVIEW These used to be in one of MFC's rc files - is there some other place to put them?
...
inline COleDateTime &COleDateTime::operator=(const UDATE &udate)
{
	...
	m_status = tmp.GetAsSystemTime(st) && ::SystemTimeToVariantTime(&st, &m_dt) ? valid : invalid;
...
inline CString COleDateTime::Format(LPCTSTR pFormat, bool bLocal) const
{
	// If null, return empty string
	if(GetStatus() == null)
		return _T("");
	...
	return bLocal ? tmTmp.Format(pFormat) : tmTmp.FormatGmt(pFormat);
}
