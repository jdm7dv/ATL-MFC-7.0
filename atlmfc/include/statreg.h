...
// DEPRECATED will be removed before RTM.
...
public:
	...
	HRESULT Add(LPCOLESTR lpszKey, LPCOLESTR lpszValue)
	{
		...
		{
			size_t cbKey = (ocslen(lpszKey)+1)*sizeof(OLECHAR);
			size_t cbValue = (ocslen(lpszValue)+1)*sizeof(OLECHAR);
			pExpand->szKey = (LPOLESTR)::CoTaskMemAlloc(UINT(cbKey));
			pExpand->szValue = (LPOLESTR)::CoTaskMemAlloc(UINT(cbValue));
			...
			else
			{
				...
				if (m_cEls == m_nSize)
				{
					...
					if (m_nSize == 0)
						m_nSize = 10;
					ppTemp = static_cast<EXPANDER**>(realloc(m_p, m_nSize*sizeof(EXPANDER*)));
					if (ppTemp == NULL)
						hRes = E_OUTOFMEMORY;
					else
						m_p = ppTemp;
...
class ATL_NO_VTABLE CRegObject
 : public IRegistrar
#else
class CRegObject  : public IRegistrar
#endif
{
public:
