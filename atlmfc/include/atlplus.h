...
//              ReplaceSingleQuote(pszModuleQuote, pszModule);
	...
	HRESULT VMUpdateRegistry(RGSOps* pOps, RGSStrings* rgStrings, BOOL bRegister) throw()
	{
		...
		pOps++;
		return VMUpdateRegistryRecurse(hKeyBase, pOps, rgStrings, bRegister);
	}
	...
	HRESULT VMUpdateRegistryRecurse(HKEY hKeyParent, RGSOps*& pOps, RGSStrings* rgStrings, BOOL bRegister) throw()
	{
		CRegKey  rkCur;
		...
		while (code != rgsopCloseAsParent)
		{
			...
			case rgsopOpenAsParent:
				pOps++;
				hr = VMUpdateRegistryRecurse(rkCur.m_hKey, pOps, rgStrings, bRegister);
				if (FAILED(hr))
					return hr;
				GetOpsFromDWORD(*pOps, code, p1, p2);
				if (!bRegister)
			...
			case rgsopAddStringValue:
			{
				if (bRegister)
				{
					...
					LPTSTR szReplacement, szReplacement2 = NULL;
					...
					lRes = rkCur.SetStringValue((p1 != 0) ? ((szReplacement2) ? szReplacement2 : rgBytes2.m_aT) : NULL, 
						(szReplacement) ? szReplacement : rgBytes.m_aT); 
			...
			case rgsopAddMultiStringValue:
			{
				if (bRegister)
				{
					...
					if (pszDestValue != NULL)
					{
						...
						*p = NULL;
						lRes = rkCur.SetMultiStringValue((p1 != 0) ? ((szReplacement2) ? szReplacement2 : rgBytes2.m_aT) : NULL, 
							pszDestValue); 
						if (ERROR_SUCCESS != lRes)
							return AtlHresultFromWin32(lRes);
						if (bRestoreRK)
