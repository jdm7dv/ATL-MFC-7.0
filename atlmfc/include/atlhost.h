...
ATLINLINE HRSRC AtlFindResource(HMODULE hModule, LPCSTR lpName, LPCSTR lpType)
{
	return ::FindResourceA(hModule, lpName, lpType);
}

ATLINLINE HRSRC AtlFindResource(HMODULE hModule, LPCWSTR lpName, LPCWSTR lpType)
{
...
template <class StringType, class Helper, Helper::ReturnType (*pFunc)(HINSTANCE, LPCDLGTEMPLATE, HWND,DLGPROC, LPARAM)>
	...
	if (hDlg != NULL)
	{
		...
		if (hResource != NULL)
		{
			...
			if (pDlg != NULL)
			{
				...
				lpDialogTemplate = ATL::_DialogSplitHelper::SplitDialogTemplate(pDlg, pInitData);
		...
		IUnknown** ppUnkContainer, IUnknown** ppUnkControl, REFIID iidSink, IUnknown* punkSink, BSTR bstrLic)
{
	...
	hr = ATL::CAxHostWindow::_CreatorClass::CreateInstance(NULL, __uuidof(IUnknown), (void**)&spUnkContainer);
	if (SUCCEEDED(hr))
	{
		ATL::CComPtr<IAxWinHostWindowLic> pAxWindow;
		spUnkContainer->QueryInterface(__uuidof(IAxWinHostWindow), (void**)&pAxWindow);
		ATL::CComBSTR bstrName(lpszName);
...
ATLINLINE ATLAPI AtlAxAttachControl(IUnknown* pControl, HWND hWnd, IUnknown** ppUnkContainer)
{
	...
	if (SUCCEEDED(hr))
	{
		ATL::CComPtr<IAxWinHostWindow> pAxWindow;
		spUnkContainer->QueryInterface(__uuidof(IAxWinHostWindow), (void**)&pAxWindow);
...
{
	_ATLWINMODULE.EnterWindowCriticalSection();
	ATL::WM_ATLGETHOST = RegisterWindowMessage(_T("WM_ATLGETHOST"));
	ATL::WM_ATLGETCONTROL = RegisterWindowMessage(_T("WM_ATLGETCONTROL"));
	WNDCLASSEX wc;
// first check if the class is already registered
	wc.cbSize = sizeof(WNDCLASSEX);
	BOOL bRet = ::GetClassInfoEx(ATL::_AtlBaseModule.GetModuleInstance(), ATL::CAxWindow::GetWndClassName(), &wc);
...
// register class if not
	...
	if(!bRet)
	{
		...
		wc.style = CS_DBLCLKS;
#endif
		wc.lpfnWndProc = ATL::AtlAxWindowProc;
		wc.cbClsExtra = 0;
		wc.cbWndExtra = 0;
		...
		wc.lpszClassName = ATL::CAxWindow::GetWndClassName();
		wc.hIconSm = NULL;
	...
	// first check if the class is already registered
		...
		wc.cbSize = sizeof(WNDCLASSEX);
		bRet = ::GetClassInfoEx(ATL::_AtlBaseModule.GetModuleInstance(), ATL::CAxWindow2::GetWndClassName(), &wc);
	...
	// register class if not
		...
		if(!bRet)
		{
			...
			wc.style = CS_DBLCLKS;
	#endif
			wc.lpfnWndProc = ATL::AtlAxWindowProc2;
			wc.cbClsExtra = 0;
			wc.cbWndExtra = 0;
			wc.hInstance = ATL::_AtlBaseModule.GetModuleInstance();
			wc.hIcon = NULL;
			...
			wc.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
			wc.lpszMenuName = NULL;
			wc.lpszClassName = ATL::CAxWindow2::GetWndClassName();
			wc.hIconSm = NULL;

			bRet = (BOOL)::RegisterClassEx(&wc);
		}
	}
	_ATLWINMODULE.LeaveWindowCriticalSection();
	return bRet;
}
...
{
	ATLASSERT(ATL::WM_ATLGETCONTROL != 0);
	if (pp == NULL)
		return E_POINTER;
	*pp = (IUnknown*)SendMessage(h, ATL::WM_ATLGETCONTROL, 0, 0);
	return (*pp) ? S_OK : E_FAIL;
}

ATLINLINE ATLAPI AtlAxGetHost(HWND h, IUnknown** pp)
{
	ATLASSERT(ATL::WM_ATLGETHOST != 0);
	if (pp == NULL)
		return E_POINTER;
	*pp = (IUnknown*)SendMessage(h, ATL::WM_ATLGETHOST, 0, 0);
	return (*pp) ? S_OK : E_FAIL;
}
