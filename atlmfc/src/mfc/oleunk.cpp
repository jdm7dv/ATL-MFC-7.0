...
LPCTSTR AFXAPI AfxGetIIDString(REFIID iid)
{
	...
	wsprintf(szIID, _T("Interface\\%s"), OLE2T(pszGUID));
	if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CLASSES_ROOT, szIID, 0, KEY_READ, &hKey))
	{
	...
	{
		wsprintf(szIID, _T("CLSID\\%s"), OLE2T(pszGUID));
		if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CLASSES_ROOT, szIID, 0, KEY_READ, &hKey))
		{
			RegQueryValueEx(hKey, NULL, NULL, NULL, (LPBYTE)szIID, &dwSize);
			RegCloseKey(hKey);
