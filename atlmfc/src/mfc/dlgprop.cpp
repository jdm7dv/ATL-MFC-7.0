...
CPropertyPage::CPropertyPage(UINT nIDTemplate, UINT nIDCaption, UINT nIDHeaderTitle, UINT nIDHeaderSubTitle)
{
	ASSERT(nIDTemplate != 0);
    CommonConstruct(MAKEINTRESOURCE(nIDTemplate), nIDCaption, nIDHeaderTitle, nIDHeaderSubTitle);
}

CPropertyPage::CPropertyPage(LPCTSTR lpszTemplateName, UINT nIDCaption, UINT nIDHeaderTitle, UINT nIDHeaderSubTitle)
{
	ASSERT(AfxIsValidString(lpszTemplateName));
...
void CPropertySheet::BuildPropPageArray()
{
	...
	PROPSHEETPAGE* ppsp = (PROPSHEETPAGE*)new BYTE[nBytes];
	m_psh.ppsp = ppsp;
...
void CPropertySheet::AddPage(CPropertyPage* pPage)
{
	...
	if (m_hWnd != NULL)
	{
		...
		ppsp = (PROPSHEETPAGE*)new BYTE[nBytes + pPage->m_psp.dwSize];
		memcpy(ppsp, m_psh.ppsp, nBytes);
		delete (void*)m_psh.ppsp;
		m_psh.ppsp = ppsp;
