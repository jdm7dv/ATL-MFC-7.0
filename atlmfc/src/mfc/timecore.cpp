...
CArchive& AFXAPI operator <<(CArchive& ar, CTime time)
{
	...
	ASSERT(time.GetTime() <= UINT_MAX);
	time_t timeConverted = (time_t) time.GetTime();
...
{
	// this version serializes only 32 bits
	ASSERT(timeSpan.GetTimeSpan() <= UINT_MAX);
	time_t timeConverted = (time_t) timeSpan.GetTimeSpan();
	return ar << (DWORD) timeConverted;
}
