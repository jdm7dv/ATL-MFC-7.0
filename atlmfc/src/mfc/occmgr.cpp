...
void COccManager::BindControls(CWnd* pWndParent)
{
	...
	if (pWndParent->m_pCtrlCont != NULL)
	{
		...
		POSITION pos = pWndParent->m_pCtrlCont->m_listSites.GetHeadPosition();
		while (pos != NULL)
		{
			pSite = pWndParent->m_pCtrlCont->m_listSites.GetNext( pos );
		...
		pos = pWndParent->m_pCtrlCont->m_listSites.GetHeadPosition();
		while (pos != NULL)
		{
         pSite = pWndParent->m_pCtrlCont->m_listSites.GetNext( pos );
