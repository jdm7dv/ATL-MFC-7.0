...
static void AFXAPI TraceDDE(LPCTSTR lpszPrefix, const MSG* pMsg)
{
	if (pMsg->message == WM_DDE_EXECUTE)
	{
		...
		{
			TRACE(traceWinMsg, 0, "Warning: Unable to unpack WM_DDE_EXECUTE lParam %08lX.\n",
				pMsg->lParam);
			return;
		}
		...
		TRACE(traceWinMsg, 0, _T("%s: Execute '%s'.\n"), lpszPrefix, lpszCommands);
	...
	else if (pMsg->message == WM_DDE_ADVISE)
	{
		...
		{
			TRACE(traceWinMsg, 0, "Warning: Unable to unpack WM_DDE_ADVISE lParam %08lX.\n",
				pMsg->lParam);
			return;
		}
		aItem = (ATOM)nItem;
...
void AFXAPI _AfxTraceMsg(LPCTSTR lpszPrefix, const MSG* pMsg)
{
	...
	{
#ifdef WIN64
		AfxTrace(_T("%s: hwnd=%p, msg = %hs (%p, %p)\n"),
			lpszPrefix, pMsg->hwnd, lpszMsgName,
			pMsg->wParam, pMsg->lParam);
#else
		AfxTrace(_T("%s: hwnd=0x%08X, msg = %hs (0x%08X, 0x%08X)\n"),
			lpszPrefix, pMsg->hwnd, lpszMsgName,
			pMsg->wParam, pMsg->lParam);
#endif
	}
	...
	{
#ifdef WIN64
		AfxTrace(_T("%s: hwnd=%p, msg = 0x%04X (%p, %p)\n"),
			lpszPrefix, pMsg->hwnd, lpszMsgName,
			pMsg->wParam, pMsg->lParam);
#else
		AfxTrace(_T("%s: hwnd=0x%08X, msg = 0x%04X (0x%08X, 0x%08X)\n"),
			lpszPrefix, pMsg->hwnd, lpszMsgName,
			pMsg->wParam, pMsg->lParam);
#endif
	}
