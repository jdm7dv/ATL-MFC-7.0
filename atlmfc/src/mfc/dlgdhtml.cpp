...
BEGIN_MESSAGE_MAP(CDHtmlDialog, CDialog)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	ON_REGISTERED_MESSAGE(CDHtmlDialog::WM_DESTROYMODELESS, OnDestroyModeless)
END_MESSAGE_MAP()
	...
	for (i=0; pEventMap[i].nType != DHTMLEVENTMAPENTRY_END; i++)
	{
		if (pEventMap[i].dispId == dispIdMember)
		{
			...
			if (pEventMap[i].nType == DHTMLEVENTMAPENTRY_NAME)
			{
				...
				if (bstrName && pEventMap[i].szName && !wcscmp(bstrName, T2COLE(pEventMap[i].szName)) ||
			...
			else if (pEventMap[i].nType == DHTMLEVENTMAPENTRY_CLASS)
			{
				...
				if (bstrClass && !wcscmp(bstrClass, T2COLE(pEventMap[i].szName)))
				{
					nIndexFound = i;
					break;
				}
			...
			else if (pEventMap[i].nType == DHTMLEVENTMAPENTRY_TAG)
			{
				...
				if (bstrTagName && !_wcsicmp(bstrTagName, T2COLE(pEventMap[i].szName)))
				{
					nIndexFound = i;
					break;
				}
	...
	DISPPARAMS *pdispparams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	for (int i=0; pEventMap[i].nType != DHTMLEVENTMAPENTRY_END; i++)
	{
		...
		{
			USES_CONVERSION;
			if (pEventMap[i].szName && !wcscmp(T2COLE(pEventMap[i].szName), bstrId))
			{
				// found the entry
