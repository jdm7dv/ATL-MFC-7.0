	...
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		if (GetVersion() & 0x80000000)
		{
			...
			::MessageBoxA(NULL,
				...
				"on Windows 95, Windows 98, or Windows ME.  It takes advantage "
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		::GetSystemDirectory(szFormat, MAX_PATH);
		lstrcat(szFormat, _T("\\MFC70%s.DLL"));
		hLangDLL = AfxLoadLangResourceDLL(szFormat);
