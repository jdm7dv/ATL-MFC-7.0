...
{
   if (m_pCtrlCont->m_pSiteCapture == this)
   {
...
BOOL COleControlSite::DestroyControl()
{
   ...
   if( m_hWnd != NULL )
   {
	...
	pos = m_pCtrlCont->m_listSites.Find( this );
	if( pos != NULL )
	{
		m_pCtrlCont->m_listSites.RemoveAt( pos );
	}
   ...
   LPCRECT prect, BOOL bErase )
{
   ...
   else
   {
	  ...
	  if( rect != *prect )
	  {
         TRACE(traceOle, 0,  "Control tried to invalidate pixels outside its bounds.  What a goober.\n" );
	  }
   }
					...
					while (pos)
					{
							  ...
							  if (m_pDynamicAccessor->GetColumnType(nCol, &dbType) == FALSE)
						{
							...
							continue;
							  ...
							  case DBTYPE_STR:
									var.ChangeType(VT_BSTR);
#pragma message ("review : cast m_pDynamicAccessor->m_pColumnInfo[nCol].ulColumnSize to int")
