...
void AFXAPI DDX_Text(CDataExchange* pDX, int nIDC, DB_NUMERIC& value)
{
	...
	if (pDX->m_bSaveAndValidate)
	{
		...
		pszText = T2COLE(LPCTSTR(strText));
		pbDigits = LPBYTE(_alloca(nLength));
		parse.cDig = nLength;
		...
		parse.nBaseShift = 0;
		parse.nPwr10 = 0;
		hResult = VarParseNumFromStr(LPOLESTR(pszText), ::GetThreadLocale(), 
			NUMPRS_STD&~NUMPRS_HEX_OCT, &parse, pbDigits);
		if (FAILED(hResult))
		{
			pDX->Fail();
		}
	...
	else
	{
		...
		n.nLo = *(ULONGLONG*)&value.val[0];
		n.nHi = *(ULONGLONG*)&value.val[8];
