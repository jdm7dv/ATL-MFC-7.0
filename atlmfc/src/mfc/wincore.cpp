...
#pragma comment(linker, "/delayload:oleacc.dll")
...
#else
	//REVIEW: /Wp64
	return ::HtmlHelp(hWnd, szHelpFilePath, nCmd, (DWORD)dwData);
#endif
}
...
LDispatch:
	...
	case AfxSig_v_v_ii:
		(this->*mmf.pfn_v_i_i)((short)LOWORD(lParam), (short)HIWORD(lParam));
		break;
	...
	case AfxSig_v_uu_M:
		(this->*mmf.pfn_v_u_u_M)(LOWORD(wParam), HIWORD(wParam), CMenu::FromHandle(reinterpret_cast<HMENU>(lParam)));
		break;
...
//OLD
	...
	case AfxSig_vww2:
		(this->*mmf.pfn_vww)((UINT)LOWORD(wParam), (UINT)HIWORD(wParam));
		break;
...
LRESULT CWnd::OnActivateTopLevel(WPARAM wParam, LPARAM)
{
	...
	{
		AFX_MODULE_THREAD_STATE* pModuleThreadState = AfxGetModuleThreadState();
		if (!(pModuleThreadState->m_lastInfo.uFlags & TTF_ALWAYSTIP))
			CancelToolTips(TRUE);
	}
    ...
    case -5018:
        {
            ...
            hr = pThis->accDoDefaultAction(i1);
            break;
        }
    case -5017:
        {
    ...
    case -5015:
        {
            ...
            hr = pThis->accLocation(i1, i2, i3, i4, i5);
            break;
        }
    case -5014:
        {
    ...
    case -5013:
        {
            ...
            hr = pThis->get_accDefaultAction(i1, i2);
            break;
        }
    case -5012:
        {
            VARIANT* i1 = (VARIANT*) V_VARIANTREF(pVarResult);
            hr = pThis->get_accSelection(i1);
            break;
        }
    case -5011:
        {
            VARIANT* i1 = (VARIANT*) V_VARIANTREF(pVarResult);
            hr = pThis->get_accFocus(i1);
            break;
        }
    case -5010:
        {
            VARIANT i1 = pDispParams->rgvarg[0];
            BSTR* i2 = V_BSTRREF(pVarResult);
            hr = pThis->get_accKeyboardShortcut(i1, i2);
            break;
        }
    case -5009:
        {
            ...
            hr = pThis->get_accHelpTopic(i1, i2, i3);
            break;
        }
    case -5008:
        {
    ...
    case -5005:
        {
            VARIANT i1 = pDispParams->rgvarg[0];
            BSTR* i2 = V_BSTRREF(pVarResult);
            hr = pThis->get_accDescription(i1, i2);
            break;
        }
    case -5004:
        {
    ...
    case -5002:
        {
            ...
            hr = pThis->get_accChild(i1, i2);
            break;
        }
    case -5001:
        {
            long* i1 = (long*) V_I4REF(pVarResult);
            hr = pThis->get_accChildCount(i1);
            break;
        }
    case -5000:
        {
            IDispatch** i1 = V_DISPATCHREF(pVarResult);
            hr = pThis->get_accParent(i1);
            break;
        }
    default:
...
{
	static LPOLESTR names[] = { L"accParent", L"accChildCount", L"accChild", L"accName", L"accValue", L"accDescription", L"accRole", L"accState", L"accHelp", L"accHelpTopic", L"accKeyboardShortcut", L"accFocus", L"accSelection", L"accDefaultAction", L"accSelect", L"accLocation", L"accNavigate", L"accHitTest", L"accDoDefaultAction" };
    static DISPID dids[] = { -5000, -5001, -5002, -5003, -5004, -5005, -5006, -5007, -5008, -5009, -5010, -5011, -5012, -5013, -5014, -5015, -5016, -5017, -5018 };
...
HRESULT CWnd::EnsureStdObj()
{
	...
	{
		HRESULT hr = CreateStdAccessibleObject(m_hWnd, OBJID_CLIENT, __uuidof(IAccessible), (void**)&m_spStdObject);
		if (FAILED(hr))
			return hr;
	}
	return S_OK;
...
{
	ASSERT(m_spStdObject != NULL);
	return m_spStdObject->put_accName(varChild, szValue);
}
...
HRESULT CWnd::CreateAccessibleProxy(WPARAM wParam, LPARAM lParam, LRESULT *plRet)
{
	ASSERT(plRet != NULL);
		...
		if (SUCCEEDED(hr))
		{
			if (m_pProxy == NULL)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					{
						m_pProxy = spProx;
						spProx->SetServer(static_cast<IAccessible*>(&m_xAccessible), static_cast<IAccessibleServer*>(&m_xAccessibleServer));
...
long CWnd::GetWindowLessChildCount()
{
 	...
 	if (m_pCtrlCont != NULL)
	{
		...
		POSITION pos;
		pos = m_pCtrlCont->m_listSites.GetHeadPosition();
		while ((pos != NULL))
		{
			COleControlSite *pSite = m_pCtrlCont->m_listSites.GetNext(pos);
			if (pSite->m_bIsWindowless)
			{
				lCount ++;
			}
		}
...
HRESULT CWnd::GetAccessibilityLocation(VARIANT varChild, long *pxLeft, long *pyTop, long *pcxWidth, long *pcyHeight)
{
	...
	if (FAILED(hr))
	{
		...
		if (varChild.lVal <= lCount + lWindowlessCount)
		{
			...
			while ( pos != NULL)
			{
				pSite = m_pCtrlCont->m_listSites.GetNext(pos);
				if (pSite->m_bIsWindowless)
				{
					lCount ++;
				}
