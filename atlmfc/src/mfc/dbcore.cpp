...
// REVIEW: TCHAR? Who uses this? (nobody in this file!)
...
void CDatabase::GetConnectInfo()
{
	...
	if (m_bUpdatable)
	{
		...
		if (Check(nRetCode) && nResult == 1)
			...
			// REVIEW: Not sure what was going on here...
	...
	CDBVariant& varValue, short nFieldType)
{
	...
	LONG_PTR nActualSize = GetData(m_pDatabase, m_hstmt, nIndex,
		nFieldType, pvData,
		varValue.m_dwType == DBVT_WSTRING ? nLen * sizeof(WCHAR) : nLen,
		m_rgODBCFieldInfos[nIndex - 1].m_nSQLType);
	...
	short nFieldType, INT_PTR* pnLen, short nSQLType, SQLULEN nPrecision)
{
	...
	case SQL_C_CHAR:
		...
		*pnLen = GetTextLen(nSQLType, nPrecision);
		...
		pvData = varValue.m_pstringA->GetBufferSetLength(*pnLen);  //REVIEW
		break;
	...
	case SQL_C_WCHAR:
		...
		*pnLen = GetTextLen(nSQLType, nPrecision);
		...
		pvData = varValue.m_pstringW->GetBufferSetLength(*pnLen); //REVIEW
		break;
...
inline static void PASCAL GetLongCharDataAndCleanup(CDatabase* pdb,
	...
	INT_PTR nLen, StringType& strValue, short nSQLType, short nSQLCType)
{
	RETCODE nRetCode;

	// Release the buffer now that data has been fetched
	strValue.ReleaseBuffer((size_t)(nActualSize < nLen ? nActualSize : nLen));  //REVIEW

	// If long data, may need to call SQLGetData again
	if (nLen <= nActualSize &&
		// REVIEW: should we be retrieving LONGVARBINARY data into a CStringW?
		(nSQLType == SQL_WLONGVARCHAR || nSQLType == SQL_LONGVARCHAR ||
		nSQLType == SQL_LONGVARBINARY))
	{
		// Reallocate the size (this will copy the data)
		*ppvData = strValue.GetBufferSetLength((size_t)nActualSize + 1);  //REVIEW
		...
		*ppvData = (BYTE*)*ppvData + nOldLen;
		...
		// Release the buffer now that data has been fetched
		strValue.ReleaseBuffer((size_t)(nActualSize + nOldLen));  //REVIEW
	}
}

void PASCAL CRecordset::GetLongCharDataAndCleanup(CDatabase* pdb, HSTMT hstmt,
	short nFieldIndex, SQLLEN nActualSize, LPVOID* ppvData, INT_PTR nLen,
