...
COleControlContainer::~COleControlContainer()
{
   ...
   pos = m_listSites.GetHeadPosition();
   while( pos != NULL )
   {
      posFollow = pos;
      pSite = m_listSites.GetNext( pos );
      ASSERT( pSite != NULL );
      if( pSite->m_pDataSourceControl == NULL )
      {
         m_listSites.RemoveAt( posFollow );
         delete pSite;
      }
   }
...
void COleControlContainer::BroadcastAmbientPropertyChange(DISPID dispid)
{
	...
	posSite = m_listSites.GetHeadPosition();
	while( posSite != NULL )
	{
		pSite = m_listSites.GetNext( posSite );
		ASSERT( pSite != NULL );
		if( pSite->m_pObject != NULL )
		{
			pControl = NULL;
			pSite->m_pObject->QueryInterface( IID_IOleControl, (void**)&pControl );
			if( pControl != NULL )
			{
...
{
	POSITION pos = m_listSites.GetHeadPosition();
	while (pos != NULL)
	{
		COleControlSite* pSite;
   ...
   case WM_MBUTTONDBLCLK:
      ...
      else
      {
         ...
         while ((pos != NULL) && (pMouseMessageSite == NULL))
         {
            ...
            {
               ASSERT( pSite->m_pWindowlessObject != NULL );

               CPoint point((DWORD)lParam);

               if (pSite->m_rect.PtInRect(point))
               {
                  pMouseMessageSite = pSite;
               }
            }
         }
      }
...
BOOL COleControlContainer::OnPaint( CDC* pDC )
{
   ...
   while( pos != NULL )
   {
      ...
      {
         pViewObject = NULL;
         pSite->m_pObject->QueryInterface( IID_IViewObject, (void**)&pViewObject );
         if( pViewObject != NULL )
         {
	...
	LPENUMUNKNOWN* ppEnumUnknown)
{
	...
	*ppEnumUnknown = NULL;
	...
	TRY
	{
		...
		{
			cObjects = pThis->m_listSites.GetCount();
			ppUnk = new LPUNKNOWN[cObjects];
			UINT_PTR i = 0;
			POSITION pos = pThis->m_listSites.GetHeadPosition();
			COleControlSite* pSite;
			while (pos != NULL)
			{
            pSite = pThis->m_listSites.GetNext( pos );
				ASSERT(pSite->m_pObject != NULL);
