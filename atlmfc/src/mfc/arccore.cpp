...
CArchive& CArchive::operator<<(__int64 dwdw)
{
	if (m_lpBufCur + sizeof(__int64) > m_lpBufMax)
		Flush();

	if (!(m_nMode & bNoByteSwap))
		_AfxByteSwap(dwdw, m_lpBufCur);
	else
		*(__int64*)m_lpBufCur = dwdw;
...
{
	if (m_lpBufCur + sizeof(__int64) > m_lpBufMax)
		FillBuffer(sizeof(__int64) - (UINT)(m_lpBufMax - m_lpBufCur));

	w = *(__int64*)m_lpBufCur;
	m_lpBufCur += sizeof(__int64);

	if (!(m_nMode & bNoByteSwap))
		_AfxByteSwap(__int64, (BYTE*)&w);
...
{
	USES_CONVERSION;
	return CRuntimeClass::FromName(W2A(lpszClassName));
}
