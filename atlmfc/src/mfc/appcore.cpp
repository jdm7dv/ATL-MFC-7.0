...
static HINSTANCE _AfxLoadLangDLL(LPCTSTR pszFormat, LCID lcid)
{
	...
	wsprintf(szLangDLL, pszFormat, szLangCode);
...
HINSTANCE CWinApp::LoadAppLangResourceDLL()
{
	...
	::GetModuleFileName(m_hInstance, szFormat, MAX_PATH);
	pszExtension = ::PathFindExtension(szFormat);
	lstrcpy(szExtension, pszExtension);
	lstrcpy(pszExtension, _T("%s.dll"));
...
void CWinApp::Dump(CDumpContext& dc) const
{
	...
	dc << "\nm_hDevMode = " << (UINT_PTR)m_hDevMode;
	dc << "\nm_hDevNames = " << (UINT_PTR)m_hDevNames;
	dc << "\nm_dwPromptContext = " << m_dwPromptContext;
	dc << "\nm_bUseHtmlHelp = " << m_bUseHtmlHelp;
	...
	dc << "\nm_hcurWaitCursorRestore = " << (UINT_PTR)m_hcurWaitCursorRestore;
