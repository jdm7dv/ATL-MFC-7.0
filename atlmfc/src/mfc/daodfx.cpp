	...
	case AFX_RFX_LONGBINARY:
		{
			...
			memcpy(pbDest, pbSrc, (size_t)pLongBinarySrc->m_dwDataLength);  //REVIEW
	...
	case AFX_RFX_BINARY:
		{
			...
			else
			{
				...
				if (memcmp(pByteArrayDest->GetData(), pByteArraySrc->GetData(), (size_t)nSize) != 0)  //REVIEW
					bDirty = TRUE;
			}
		}
		break;
	...
	case AFX_RFX_LONGBINARY:
		{
			...
			{
				// If sizes compare, compare the data
				if (memcmp(pbDest, pbSrc, (size_t)pLongBinarySrc->m_dwDataLength) != 0)  //REVIEW
					bDirty = TRUE;
			}
