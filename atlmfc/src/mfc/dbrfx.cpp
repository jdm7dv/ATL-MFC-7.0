...
{
inline int RFX_Text_strlen(LPSTR  psz){return ::lstrlenA(psz);}
inline int RFX_Text_strlen(LPWSTR psz){return ::lstrlenW(psz);}
...
inline void RFX_Text_inc(LPSTR *ppsz)
		...
		{(*ppsz)++;}
	#endif
inline void RFX_Text_inc(LPWSTR *ppsz){(*ppsz)++;}
	...
	short nFieldType, CharType cSpace)
{
	...
	ASSERT(AfxIsValidAddress(value, nMaxLength * sizeof(CharType)));
...
// REVIEW This is a workaround for Bug #63286
CStringA g_dummyA; // instantiate the code for CStringA
CStringW g_dummyW;
...
void RFX_Text(CFieldExchange* pFX, LPCTSTR szName,
	...
	short nFieldType, StringType::XCHAR cSpace)
{
	ASSERT(AfxIsValidAddress(pFX, sizeof(CFieldExchange)));
	ASSERT(AfxIsValidString(szName));
	ASSERT(AfxIsValidAddress(&value, sizeof(StringType)));
	...
	case CFieldExchange::BindFieldToColumn:
		{
			...
			// Set up binding addres
			void* pvData;
			value.GetBufferSetLength((size_t)cbColumn+1);  //REVIEW
...
void AFXAPI RFX_Text(CFieldExchange* pFX, LPCTSTR szName,
	LPWSTR value, int nMaxLength, int nColumnType, short nScale)
{
	nsRFX_Text::RFX_Text(pFX, szName, value, nMaxLength, nColumnType, nScale, SQL_C_WCHAR, L' ');
}

void AFXAPI RFX_Text(CFieldExchange* pFX, LPCTSTR szName,
	LPSTR value, int nMaxLength, int nColumnType, short nScale)
{
...
#ifdef _DEBUG
				TRACE(traceDatabase, 1, "Warning: date value out of range, returning NULL value.\n");
#endif
				pFX->m_prs->SetNullFieldStatus(nField - 1);
				value = AFX_RFX_DATE_PSEUDO_NULL;
			}
...
inline void RFX_Text_Bulk(CFieldExchange* pFX, LPCTSTR szName,
	CharType **prgStrVals, LONG_PTR** prgLengths, int nMaxLength,
	short nFieldType)
{
	ASSERT(AfxIsValidAddress(pFX, sizeof(CFieldExchange)));
	ASSERT(AfxIsValidString(szName));
	...
	case CFieldExchange::AllocMultiRowBuffer:
		{
			...
			*prgStrVals = new CharType[nRowsetSize * nMaxLength]; // REVIEW: Make sure bulk-row fetching works for Unicode
